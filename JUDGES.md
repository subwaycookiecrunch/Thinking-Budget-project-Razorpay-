...
| **AI Judgment** | Use the model for source interpretation; use code for allocation and evidence checks | Real model mode plus labeled offline fallback | Saved research training exists; product uses a separate pretrained model and claims no superiority |
...
The evaluation produces `grpo_output/benchmark_results.json`, per-episode JSONL, and a Pareto plot. Its policy comparison measures files actually read. It reports covered and unread positive labels, safe-file reads, and source-character cost; it does not measure bug detection. The generated metadata is the source of truth for policy settings, dataset identity, and uncertainty.
...
Historical `trained`/`untrained` trace names are legacy labels for generated policies. Historical F1 improvements, calibration plots, and apparent tag-removal results do not prove learned behavior. **Newly supplied Qwen2.5-1.5B LoRA weights and a 100-step run are present.** The training summary reports mean reward 0.094827, but all recorded action coupling is zero; efficacy remains unproven. The product runs pretrained Qwen3:4b and does not use this adapter.
...
A fair model comparison needs real completions under fixed and adaptive budgets, the same base model and test inputs, actual token/latency measurements, and labels independent of policy development. A training-improvement claim additionally needs baseline-versus-adapter inference on independent test inputs; weights alone are insufficient. A business-value claim needs developer pilot data. Those are specific next validations, not completed achievements.
